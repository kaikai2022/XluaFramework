...
static void printjmp (const InstructionLpeg *op, const InstructionLpeg *p) {
  printf("-> %d", (int)(p + (p + 1)->offset - op));
}


void printinst (const InstructionLpeg *op, const InstructionLpeg *p) {
...
void printpatt (InstructionLpeg *p, int n) {
  InstructionLpeg *op = p;
  while (p < op + n) {
    printinst(op, p);
    p += sizei(p);
  }
