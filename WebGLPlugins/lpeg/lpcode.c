...
*/
int sizei (const InstructionLpeg *i) {
...
void realloccode (lua_State *L, Pattern *p, int nsize) {
  ...
  void *newblock = f(ud, p->code, p->codesize * sizeof(InstructionLpeg),
                                  nsize * sizeof(InstructionLpeg));
  if (newblock == NULL && nsize > 0)
    luaL_error(L, "not enough memory");
  p->code = (InstructionLpeg *)newblock;
  p->codesize = nsize;
}
...
*/
static int finaltarget (InstructionLpeg *code, int i) {
...
*/
static int finallabel (InstructionLpeg *code, int i) {
...
static void correctcalls (CompileState *compst, int *positions,
                          int from, int to) {
  int i;
  InstructionLpeg *code = compst->p->code;
...
** Compile a pattern
*/
InstructionLpeg *compile (lua_State *L, Pattern *p) {
  CompileState compst;
