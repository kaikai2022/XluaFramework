...
public class MVCTools : EditorWindow
{
    ...
    public void OnGUI()
    {
        ...
        GUILayout.Space(5);
        GUILayout.Label("作者");
        author = GUILayout.TextArea(author, 120);

        GUILayout.Space(7);
        GUILayout.Label("父目录  命名格式:XXX");
        if (GUILayout.Button("保存新路径"))
        {
            if (parentPath == null || parentPath.Equals("") || parentPath.IsNullOrEmpty())
            {
                DirectoryName = "Assets" + Path.DirectorySeparatorChar + "LuaScripts" + Path.DirectorySeparatorChar +
                                "UI" + Path.DirectorySeparatorChar;
                return;
            }

            DirectoryName = "Assets" + Path.DirectorySeparatorChar + "LuaScripts"
        ...
        GUILayout.Space(1);
        GUILayout.Label(string.Format("当前目录路径：{0}", DirectoryName));
        parentPath = GUILayout.TextArea(parentPath, 120);


        GUILayout.Space(5);
        GUILayout.Label("UI名字  命名格式:UIXXX");
        newUIStr = GUILayout.TextArea(newUIStr, 120);
        GUILayout.Label("文件夹名名字");
        newFileStr = GUILayout.TextArea(newFileStr, 120);
        GUILayout.Space(10);
    ...
    private static void CreatWrapItem()
    {
        ...
        if (!Directory.Exists(DirectoryName + Path.DirectorySeparatorChar + newFileStr + Path.DirectorySeparatorChar +
                              ComponentName))
        {
            Directory.CreateDirectory(DirectoryName + Path.DirectorySeparatorChar + newFileStr +
                                      Path.DirectorySeparatorChar + ComponentName);
        }

        string luaData = "";
        luaData = "--[[\r\n-- added by " + author + " @" + System.DateTime.Now + "\r\n-- +" + newFileStr + "模块" +
                  newFileStr + "View窗口中服务器列表的可复用Item\r\n--]]\r\n"
        ...
        string path = DirectoryName + newFileStr + Path.DirectorySeparatorChar + ComponentName +
                      Path.DirectorySeparatorChar + newWarpItemStr + ".lua";
    ...
    private static void CreatCrtlLua()
    {
        ...
        Debug.Log(luaData);
        string path = DirectoryName + newFileStr + Path.DirectorySeparatorChar + ControllerName +
                      Path.DirectorySeparatorChar + newUIStr + "Ctrl.lua";
        CreatUIScript(path, luaData);
    }
    ...
    private static void CreatModellLua()
    {
        ...
        Debug.Log(luaData);
        string path = DirectoryName + newFileStr + Path.DirectorySeparatorChar + ModelName +
                      Path.DirectorySeparatorChar + newUIStr + "Model.lua";
        CreatUIScript(path, luaData);
    }
    ...
    private static void CreatViewlLua()
    {
        ...
        Debug.Log(luaData);
        string path = DirectoryName + newFileStr + Path.DirectorySeparatorChar + ViewName +
                      Path.DirectorySeparatorChar + newUIStr + "View.lua";
        CreatUIScript(path, luaData);
    }
    ...
    {
        string parentStr = parentPath.IsNullOrEmpty() || DirectoryName.IndexOf(parentPath) < 0 ? "" : parentPath.Replace("/",".") + ".";
        string luaData = "";
        string model = (isModel)
            ? ("Model = require " + "\"" + parentStr + "UI." + newUIStr + ".Model." + newUIStr + "Model\",")
            : (" Model =nil,");
        string view = (isView)
            ? ("View = require " + "\"" + parentStr + "UI." + newUIStr + ".View." + newUIStr + "View\",")
            : (" View =nil,");
        string crtl = (isCrtl)
            ? ("Ctrl =  require " + "\"" + parentStr + "UI." + newUIStr + ".Controller." + newUIStr + "Ctrl\",")
            : (" Ctrl =nil,");
        luaData = "--[[\r\n"
                  + "-- added by " + author + " @ " + System.DateTime.Now + "\r\n"
    ...
    public static string GetConfigPath = "Assets" + Path.DirectorySeparatorChar + "LuaScripts" +
                                         Path.DirectorySeparatorChar + "UI" + Path.DirectorySeparatorChar + "Config" +
                                         Path.DirectorySeparatorChar
                                         + "UIConfig.lua";
    ...
    public static string GetUIWindowsPath = "Assets" + Path.DirectorySeparatorChar + "LuaScripts" +
                                            Path.DirectorySeparatorChar + "UI" + Path.DirectorySeparatorChar +
    ...
    static string DirectoryName = "Assets" + Path.DirectorySeparatorChar + "LuaScripts" + Path.DirectorySeparatorChar +
                                  "UI" + Path.DirectorySeparatorChar;
