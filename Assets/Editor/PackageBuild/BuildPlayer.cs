...
//        var old_manifest = GetCurrentManifest();
        Logger.Log(String.Format("BuildAssBundles OutputPath: {0}", outputPath));
...
//        SaveCDNFlushFile(updates);
    ...
    private static void ClearUnuseFiles(string outputPath, AssetBundleManifest manifest)
    {
        ...
        //clear no use files
        string[] ignoredFiles =
            {"AssetBundles", "AssetBundleServerUrl", "versions.bytes", "assetsmap_bytes", "app_version.bytes"};
    ...
    public static void BuildAssetBundles(BuildTarget buildTarget, string channelName)
    {
        ...
        Debug.Log(string.Format("Build assetbundles for platform : {0} and channel : {1} done!", targetName,
            channelName));
    }
    ...
    public static void BuildAssetBundlesForAllChannels(BuildTarget buildTarget)
    {
        ...
        foreach (var current in (ChannelType[]) Enum.GetValues(typeof(ChannelType)))
        {
            ...
            Debug.Log(string.Format(
                "{0}.Build assetbundles for platform : {1} and channel : {2} done! use time : switchChannel = {3}s , build assetbundls = {4} s",
                index, targetName, channelName, switchChannel, buildAssetbundles));
        }
    ...
    public static void BuildXCode(string channelName, bool isTest)
    {
        ...
        {
            Texture2D texture =
                (Texture2D) AssetDatabase.LoadAssetAtPath(string.Format(iconPath, channelName, iconSizes[i]),
                    typeof(Texture2D));
