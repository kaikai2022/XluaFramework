...
namespace UI
{
    ...
    public class GridPageView : MonoBehaviour, IEndDragHandler
    {
            ...
            UpdateGridItem updateItem = null)
        {
            ...
            contentRectTf.pivot = new Vector2(0, 1);
            contentRectTf.anchorMin = new Vector2(0, 1);
            contentRectTf.anchorMax = new Vector2(0, 1);
            contentRectTf.anchoredPosition3D = Vector3.zero;
        ...
        private void createPageItem(GameObject prefab, int createPageCount)
        {
            ...
            for (int i = 0; i < createPageCount; ++i)
            {
                ...
                pageContainer.transform.rectTransform().anchoredPosition3D = Vector3.zero;
                ...
                {
                    GameObject item =
                        MonoBehaviour.Instantiate(prefab, new Vector3(0, 0, 0), new Quaternion()) as GameObject;


                    var rectTransform = item.transform.rectTransform();
                    ...
                    item.transform.rectTransform().anchoredPosition3D = new Vector3(x, y, 0);
                    columns++;
        ...
        private void fixContentPos()
        {
            if (!this.isHorizontal)
            {
                ...
                if (this.contentRectTf.sizeDelta.y <= this.pageSize.y)
                {
                    ...
                    if (this.contentRectTf.anchoredPosition.y > 0)
                        this.contentRectTf.anchoredPosition = new Vector2(this.contentRectTf.anchoredPosition.x, 0);
                }
                else
                {
                    //如果高度足够但content底部超过scroll的底部则content底部对齐scroll的底部
                    if (this.contentRectTf.anchoredPosition.y - this.contentRectTf.sizeDelta.y > -this.pageSize.y)
                        this.contentRectTf.anchoredPosition = new Vector2(this.contentRectTf.anchoredPosition.x,
            ...
            else
            {
                ...
                {
                    if (this.contentRectTf.anchoredPosition.x < 0)
                        this.contentRectTf.anchoredPosition = new Vector2(0, this.contentRectTf.anchoredPosition.y);
                }
                else
                {
                    if (this.contentRectTf.anchoredPosition.x + this.contentRectTf.sizeDelta.x < this.pageSize.x)
                        this.contentRectTf.anchoredPosition = new Vector2(
                            this.pageSize.x - this.contentRectTf.sizeDelta.x,
                            this.contentRectTf.anchoredPosition.y);
                }
            }
        }
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            //计算当前页未拖动时的位置
            Vector2 beforeDragPagePos =
                new Vector2(-this.curPageIndex * this.pageSize.x, this.curPageIndex * this.pageSize.y);
            //获取拖动后的位置
            Vector2 afterDragPagePos = this.content.transform.rectTransform().anchoredPosition;
            if (this.isHorizontal)
            {
                //横向
                if (afterDragPagePos.x < beforeDragPagePos.x)
                {
                    ...
                    if (afterDragPagePos.x < beforeDragPagePos.x - this.pageSize.x * .5f)
                        this.contentRectTf.DOAnchorPosX(beforeDragPagePos.x - this.pageSize.x - .01f, .4f); //翻页
                    else
                        this.contentRectTf.DOAnchorPosX(beforeDragPagePos.x, .4f); //回滚
                    // this.contentRectTf.DOAnchorPosX(beforeDragPagePos.x, .4f);
                }
            ...
            else
            {
                ...
                if (afterDragPagePos.y > beforeDragPagePos.y)
                {
                    ...
                    if (afterDragPagePos.y > beforeDragPagePos.y + this.pageSize.y * .5f)
                        this.contentRectTf.DOAnchorPosY(beforeDragPagePos.y + this.pageSize.y + .01f, .4f); //翻页
                    else
                        this.contentRectTf.DOAnchorPosY(beforeDragPagePos.y, .4f); //回滚
                }
        ...
        private void updatePage()
        {
            ...
            for (int i = 0; i < this.pageList.Count; ++i)
            {
                ...
                if (this.isHorizontal)
                {
                    if (pagePos.x < this.left * 1.5 && this.curPageIndex < this._pageCount - this.showPageCount)
                    {
                        //向左循环
                    ...
                    else if (pagePos.x > this.right * 0.5 && this.curPageIndex > 0)
                    {
                        //向右循环
                ...
                {
                    if (pagePos.y > this.top * 1.5 && this.curPageIndex < this._pageCount - this.showPageCount)
                    {
                        //向上循环
                        GameObject lastPageGo = this.pageList[this.pageList.Count - 1];
                        this.pageList.RemoveAt(i);
                        pageGoTr.localPosition = new Vector3(pageGoTr.localPosition.x,
                            lastPageGo.transform.localPosition.y - this.pageSize.y);
                        this.pageList.Add(pageGo);
                        pageGo.name = "page" + (this.curPageIndex + this.showPageCount);
                    ...
                    else if (pagePos.y < this.bottom * 0.5 && this.curPageIndex > 0)
                    {
                        //向下循环
                        GameObject firstPageGo = this.pageList[0];
                        this.pageList.RemoveAt(i);
        ...
        private Vector2 getFirstPagePos()
        {
            ...
            return pageGo.GetComponent<RectTransform>().anchoredPosition;
        }
        ...
        private void layoutPage()
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                GameObject pageGo = this.pageList[i];
                if (!this.isHorizontal)
                    pageGo.transform.rectTransform().anchoredPosition =
                        new Vector2(0, this.firstPagePos.y - this.pageSize.y * i);
                else
                    pageGo.transform.rectTransform().anchoredPosition =
                        new Vector2(this.firstPagePos.x + this.pageSize.x * i, 0);
            }
        }
        ...
        public void rollPosByPage(int page)
        {
            ...
            Vector2 contentPos = this.contentRectTf.anchoredPosition;
            if (!this.isHorizontal)
            {
            ...
            this.contentRectTf.anchoredPosition = contentPos;
